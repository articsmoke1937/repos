...
globals.pname=up.last_user(globals.user_log_filename)

#check user name to see if it is in the global user log
globals.user_log=up.username_check(globals.users_file,globals.pname)

#GUI 
# root = Tk()
# main_gui = GUIMaster.app_gui(root)
root = gm.app_gui.start()
gm.PageContainer.show_frame(gm.validation_page())
root.mainloop()
...
# # #validation gui
# # root=Tk()
# # validation_gui=GUIMaster.app_gui.user_val_win(pname)
# # root.mainloop()
